...
import com.exmart.qiyishow.record.ProgressView.State;
import com.exmart.qiyishow.record.ProgressView;
...
public class FFmpegRecorderActivity extends Activity implements OnClickListener, OnTouchListener {
	...
	private TextView switchCameraIcon;
	private TextView recorder_state;
	private boolean stateFlag = false;
	private Button recorder_video;
	...
	private int previewHeight = 640, screenHeight = 640;
	...
	private void initLayout()
	{
		...
		cancelBtn = (TextView) findViewById(R.id.recorder_cancel);
		cancelBtn.setOnClickListener(this);
		nextBtn = (Button) findViewById(R.id.recorder_next);
		nextBtn.setOnClickListener(this);
		//txtTimer = (TextView)findViewById(R.id.txtTimer);
		flashIcon = (TextView)findViewById(R.id.recorder_flashlight);
		switchCameraIcon = (TextView)findViewById(R.id.recorder_frontcamera);
		recorder_state = (TextView) findViewById(R.id.recorder_state);
		recorder_video = (Button) findViewById(R.id.recorder_video);
		recorder_video.setOnTouchListener(this);
		flashIcon.setOnClickListener(this);
		recorder_state.setOnClickListener(this);
...
//				mCamera.stopPreview();
	...
	private void initVideoRecorder() {
		...
		videoRecorder = new NewFFmpegFrameRecorder(strVideoPath, 480, 640, 1);
	...
	private void showCancellDialog(){
		Util.showDialog(FFmpegRecorderActivity.this, "提示", "要放弃本视频吗", 2, new Handler(){
			@Override
			public void dispatchMessage(Message msg) {
				if(msg.what == 1)
					videoTheEnd(false);
	...
	public void onClick(View v) {
		...
		if(v.getId() == R.id.recorder_next){
			...
			if (recording) 
				...
				videoTheEnd(false);
		}else if(v.getId() == R.id.import_video){
			Intent intent = new Intent(this, LocalVideoList.class);
			startActivity(intent);
		}else if(v.getId() == R.id.recorder_state){
			if(stateFlag){
				recorder_state.setBackgroundResource(R.drawable.video_state_standard);
				stateFlag = false;
			}else{
				recorder_state.setBackgroundResource(R.drawable.video_state_widescreen);
				stateFlag = true;
			}
