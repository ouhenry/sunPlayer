 ...
 * @since 1.7.1
 */
class VideoLoaderEngine {

	final VideoLoaderConfiguration configuration;
	...
	VideoLoaderEngine(VideoLoaderConfiguration configuration) {
	...
	void submit(final LoadVideoTask task) {
		taskDistributor.execute(new Runnable() {
			@Override
			public void run() {
				File video = configuration.diskCache.get(task.getLoadingUri());
				boolean isVideoCachedOnDisk = video != null && video.exists();
				initExecutorsIfNeed();
				if (isVideoCachedOnDisk) {
